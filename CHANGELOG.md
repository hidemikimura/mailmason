...
行・ブロックをまとめて選べるようにし、別のメールへのコピー＆貼り付けに対応しました。最初に読み込む量を減らしました。

### 追加

- 複数選択: `Shift` または `Ctrl/⌘` を押しながらクリックすると、行どうし・ブロックどうしをまとめて選べる。まとめて削除（`Delete`）・複製（`Ctrl/⌘+D`）でき、設定パネルに件数と「複製」「コピー」「切り取り」「削除」のボタンを出す。`select()` に ID の配列を渡してもまとめて選べる
- コピー＆貼り付け: 選んだ行・ブロックを `Ctrl/⌘+C` / `X` / `V` でコピー・切り取り・貼り付けできる。OS のクリップボードにテンプレート JSON を入れるので、別のタブや別のアプリで開いた Mailmason のメールにも貼り付けられる（ID は振り直す）
- core: 複数のコマンドを 1 つの履歴で適用する `batch` コマンド（型 `BatchCommand`）。ストアの状態に `selectedIds`、メソッド `toggleSelect(id)` を追加し、`select()` は ID の配列も受け付ける

### 変更

- `mm-select` の `detail` に `ids`（選択中の要素すべて）を追加
- プレビュー・表の直接編集・QR コードの作成（QR のライブラリを含む）・英語の UI 文言を、使うときに読み込むようにした。npm 版はバンドラーが別ファイルに分け、ES モジュール版（`dist/mailmason.bundle.js`）は `dist/chunks/` に分ける。ES モジュール版は空白も圧縮するようにし、最初に読み込む量は gzip で約 106KB から約 80KB になった。script タグ版（`dist/mailmason.iife.js`）はこれまでどおり 1 ファイル（約 97KB）
- 表は、選択したときに編集部品を読み込む（読み込むまでは表示だけ）。プレビューは初めて開いたときに読み込む
- `locale` が `'ja'` 以外のときは、英語の文言を読み込んでから最初の描画をする（日本語の画面が一瞬出ないように）
- `npm run build` で ES モジュール版と script タグ版を別々にビルドするようにし、`npm run size`（バンドルの大きさと上限の確認）を追加。`prepublishOnly` でも確かめる

### 互換性

- ES モジュール版（`dist/mailmason.bundle.js`）を自分のサーバーに置いている場合は、`dist/chunks/` も同じ場所に置いてください（CDN・npm 版・script タグ版は対応不要）
- テンプレート JSON の形式は変わりません。0.7.0 で保存したテンプレートは 0.6.x でもそのまま読み込めます
- `mm-select` の `detail` に `ids` が加わりました。`select()` と `store.select()` は ID の配列も受け付けます（文字列・`null` はこれまでどおり）
- コピー＆貼り付けで、行・ブロックを選んでいるときの `Ctrl/⌘+C` / `X` / `V` をエディタが扱います（入力欄・直接編集中・文字を選んでいるときはブラウザ本来の動作のまま）。行・ブロックの貼り付けでは、クリップボードの画像より Mailmason のデータを優先します
