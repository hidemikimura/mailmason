...
| `webFonts`                          | 配列                                                                                                                                                                    |                                                                              | 読み込む Web フォント（Google Fonts などの CSS）。`fontFamily`（ボディ・テキストブロック）で名前を使ったものだけ、head で読み込む。Apple Mail・iOS メール・Outlook for Mac などで表示され、Gmail・Outlook（Windows）・Yahoo!メールなどでは `fontFamily` の後ろのフォントになる。名前か URL の無い項目は読み込み時に除く |
| `webFonts[].family`                 | 文字列                                                                                                                                                                  | `""`                                                                         | フォント名（`fontFamily` に書く名前。例: `Noto Sans JP`）                                                                                                                                                                                                                                                               |
| `webFonts[].url`                    | 文字列                                                                                                                                                                  | `""`                                                                         | フォントの CSS の URL（`https://` のみ。例: `https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@400;700&display=swap`）                                                                                                                                                                                         |
| `fontSize`                          | 数値 8〜72                                                                                                                                                              | `16`                                                                         | 基本の文字サイズ（px）                                                                                                                                                                                                                                                                                                  |
| `mobileFontSize`                    | 数値 8〜72 \| `null`                                                                                                                                                    | `null`                                                                       | スマホの基本の文字サイズ（px、`null` なら PC と同じ）。文字サイズを指定していないテキスト・表・画像＋テキストなどに効く。Outlook（Windows）とメディアクエリに対応しないメールソフトでは PC と同じ                                                                                                                       |
...
| `hideOn`                     | `"mobile"` \| `"desktop"` \| `null`                                                                                                                                     | `null`                                    | `"mobile"` でスマホでは表示しない。`"desktop"` で PC では表示しない（スマホだけに出す。Outlook（Windows）とメディアクエリに対応しないメールソフトでは表示されない） |
...
| `mobileFontSize` | 数値 8〜72 \| `null`      | `null` | スマホの文字サイズ（px）。`null` なら、`fontSize` が `null` のときはボディの `mobileFontSize`、それ以外は PC と同じ |
...
| `mobileFontSize`  | 数値 8〜72 \| `null`                           | `null`                                        | スマホの文字サイズ（px、`null` なら PC と同じ） |
...
| `merges`                | 配列                                |             | 結合したセル。左上のセルの位置と、縦・横に結合する数。覆われたセルの値は出力しない。重なる結合・表からはみ出す結合は読み込み時に直す |
| `merges[].row`          | 整数 0〜49                          | `0`         | 左上のセルの行（0 から）                                                                                                             |
| `merges[].column`       | 整数 0〜7                           | `0`         | 左上のセルの列（0 から）                                                                                                             |
| `merges[].rowSpan`      | 整数 1〜50                          | `1`         | 縦に結合する行の数                                                                                                                   |
| `merges[].colSpan`      | 整数 1〜8                           | `1`         | 横に結合する列の数                                                                                                                   |
...
| `mobileLayout`          | `"table"` \| `"stack"`              | `"table"`   | スマホでの表示。`stack` は 1 行ずつ「見出し：値」の縦並びにする（メディアクエリに対応しないメーラーと Outlook では表のまま）         |
