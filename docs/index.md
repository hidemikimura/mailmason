...
features:
  - icon: 🧱
    title: 行 × カラムで組み立てる
    details: 1〜4 カラムのレイアウトに、テキスト・画像・ボタン・表・動画・画像ギャラリーなど 14 種類のブロックを置くだけ。テキストはキャンバス上でそのまま編集できます。
  - icon: 📨
    title: 崩れにくい HTML
  ...
  - icon: 🧩
    title: 自社用に広げる
    details: 商品カードやクーポンなど独自のブロックを defineBlock() で追加でき、作った行やブロックはコンポーネントとして保存して別のメールで再利用できます。
  - icon: 🔌
    title: どこにでも組み込める
